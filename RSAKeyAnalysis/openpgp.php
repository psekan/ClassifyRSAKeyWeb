...
class OpenPGP_LiteralDataPacket extends OpenPGP_Packet {
  ...
  function __construct($data=NULL, $opt=array()) {
    parent::__construct();
    $this->data = $data;
    $this->format = array_key_exists('format', $opt) ? $opt['format'] : 'b';
    $this->filename = array_key_exists('filename', $opt) ? $opt['filename'] : 'data';
    $this->timestamp = array_key_exists('timestamp', $opt) ? $opt['timestamp'] : time();
  }
